class SQL {
	...
	findOneForReg (field) {
		return "SELECT `phone` FROM `t_user` WHERE `phone`='" + field + "'";
	}

	insertOneForReg (o) {
		
		return "INSERT INTO `t_user` (`phone`,`pwd`) VALUES ('" + o.phone + "','" + o.pwd + "')";
	}

	findOneForLogin (o) {
		return "SELECT `id`,`phone` FROM `t_user` WHERE  `phone`='" + o.phone + "'";
	}
	findAllForShoppingcart (o) {
		return "SELECT * FROM `t_shopcart` WHERE `phone`='" + o.phone + "'AND `statusCode`=0";
	}
